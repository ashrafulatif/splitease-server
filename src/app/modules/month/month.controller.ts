...
const getMonthById = catchAsync(async (req: Request, res: Response) => {
	const result = await MonthService.getMonthById(req.params.id as string, req.user);
...
const deleteMonth = catchAsync(async (req: Request, res: Response) => {
	const result = await MonthService.deleteMonth(req.params.id as string, req.user);
